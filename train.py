...
from data import load_data, create_folds, TextDataset
from model import create_model_and_tokenizer
from trainer import Trainer
from ensemble import optimize_weights, ensemble_predict
...
def run_ensemble_optimization(config: Config, labels: np.ndarray):
    """Loads saved OOF predictions from disk and optimize ensemble weights"""
    ...
    # Load predictions for all defined models
    for model_key in MODELS.keys():
        model_path = output_dir / model_key / "oof_probs.npy"
        if model_path.exists():
            print(f"Loading predictions for: {model_key}")
            probs_dict[model_key] = np.load(model_path)
        else:
            missing_models.append(model_key)
    
    if missing_models:
        print(f"\nError: Cannot create ensemble. Missing results for: {', '.join(missing_models)}")
        print("Please train these models individually first.")
        return
    ...
    ensemble_preds, ensemble_probs = ensemble_predict(probs_dict, weights)
    ensemble_accuracy = np.mean(ensemble_preds == labels)
    
    np.save(ensemble_dir / "oof_probs.npy", ensemble_probs)
    
    summary = {
        "model": "Weighted Ensemble (RoBERTa + DeBERTa + HerBERT)",
        "cv_accuracy": float(ensemble_accuracy),
        "fold_accuracies": [float(ensemble_accuracy)],
        "weights": weights,
        "timestamp": datetime.now().isoformat()
    }
...
def main():
    ...
    parser.add_argument(
        "--ensemble", 
        action="store_true",
        help="Run ensemble optimization (requires all models to be trained first)"
    )
    ...
    if args.model:
        ...
        # Run ensemble logic
        run_ensemble_optimization(config, labels)
        
    else:
        print("\nSpecify an action:")
        print(f"  Train a model:   python train.py --model [{'|'.join(MODELS.keys())}]")
        print(f"  Create ensemble: python train.py --ensemble")
        sys.exit(1)
